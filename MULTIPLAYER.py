...
ctrl2 = pygame.joystick.Joystick(1)
running=True
pos = pygame.Vector2(0,0)
pos2 = pygame.Vector2(0,0)
...
async def run():
    ...
    drone2 = System(mavsdk_server_address=None, port=50052, sysid=245, compid=190)
    #startup
    ...
    async for health in drone2.telemetry.health():
        if health.is_global_position_ok and health.is_home_position_ok:
            print("-- 2nd Global position estimate OK")
            break
    ...
    await drone2.action.set_takeoff_altitude(10.0)
    ...
    print("-- Setting initial setpoint2")
    await drone2.offboard.set_velocity_body(
        VelocityBodyYawspeed(0.0, 0.0, 0.0, 0.0))
    
    print("-- Starting offboard2")
    try:
        await drone2.offboard.start()
    except OffboardError as error:
        print(f"Starting offboard mode failed with error code: \
              {error._result.result}")
        print("-- Disarming No.2")
    ...
    while running: 
        ...
        if ctrl2.get_button(0) == 1:
            print("X")
            #pygame.quit()
            #exit()
        if ctrl2.get_button(1) == 1:
            #print("A")
            alt2 += 0.5
        elif ctrl2.get_button(2) == 1:
            #print("B")
            alt2 -= 0.5
        else:
            alt2 = 0
        if ctrl2.get_button(3) == 1:
            print("Y")
        if ctrl2.get_button(4) == 1:
            #print("L")
            rot2 -= 3
        elif ctrl2.get_button(5) == 1:
            #print("R")
            rot2 += 3 
        else:
            rot2 = 0
        if ctrl2.get_button(8) == 1:
            print("SELECT")
            try:
        ...
        if ctrl2.get_button(9) == 1:
            ...
            try:
                ...
                return
        #AXES
        if round(ctrl2.get_axis(1))==-1:
            pos2.y -= 0.1
            #print("forward") 
        if round(ctrl2.get_axis(1))==1:
            pos2.y += 0.1 
            #print("back")
        if round(ctrl2.get_axis(1))==0:
            pos2.y = 0
        if round(ctrl2.get_axis(0))==-1:
            pos2.x += 0.1
            #print("left") 
        if round(ctrl2.get_axis(0))==1:
            pos2.x -= 0.1
        ...
        if round(ctrl2.get_axis(0))==0:
            pos2.x = 0
        #pos2.y = ctrl2.get_axis(0)
        #pos2.x = ctrl2.get_axis(1)
        ...
        print(round(rot2,2),round(rot,2))
        ...
        await drone2.offboard.set_velocity_body(VelocityBodyYawspeed(pos2.y, pos2.x, -alt2, rot2))
        time.sleep(0.01)
